            ...
            <tr style="border-bottom: 1px solid var(--line); transition: background 0.15s;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background='white'">
                ...
                <td style="padding: 16px 20px; text-align: center;">
                    <div style="display: flex; gap: 8px; justify-content: center;">
                        @if(!$esProtegido)
                        <!-- Editar -->
                        <button onclick="abrirEditar({{ $user->id }}, '{{ addslashes($user->name) }}', '{{ $user->email }}', '{{ $user->rol ?? '' }}')"
                        ...
                        <!-- Eliminar -->
                        <form action="/usuarios/{{ $user->id }}" method="POST" onsubmit="return confirm('¿Eliminar a {{ $user->name }}?');">
                            @csrf @method('DELETE')
                            <button type="submit"

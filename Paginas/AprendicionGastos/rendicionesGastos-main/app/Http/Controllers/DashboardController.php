...
class DashboardController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('dashboard', compact('reports', 'stats', 'statusGroup', 'fixedFundSummary', 'draftExpensesCount'));
    }
    ...
    public function analytics(Request $request)
    {
        ...
        // ── Category breakdown ─────────────────────────────────────────
        $categoryStats = \App\Models\Expense::with('category')
                ...
                if (!$isAdmin) {
                    ...
                    $q->where(function ($sq) use ($user) {
                        $sq->where('user_id', $user->id)
                           ->orWhere('status', '!=', \App\Models\Report::STATUS_DRAFT);
                    });
                }
            })
            ->get()
        ...
        foreach ($statusKeyConfig as $key => $applyStatus) {
            ...
            $expenses = \App\Models\Expense::with('category')
                ->whereHas('report', function ($rq) use ($user, $isAdmin, $applyStatus, $key) {
                    ...
                    if ($key === 'all') {
                        $rq->whereIn('status', [\App\Models\Report::STATUS_REIMBURSED, \App\Models\Report::STATUS_CLOSED]);
                    } else {
                        $applyStatus($rq);
                    }
                })->get()
        ...
        return view('analytics', compact('counts', 'amounts', 'monthly', 'categoryStats', 'recentReports', 'isAdmin', 'monthlyLabels', 'chartsByFilter'));
    }
}

...
class ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('reports.index', compact('reports', 'categories', 'statuses', 'cecos'));
    }
    ...
    public function inbox(Request $request)
    {
        ...
        return view('reports.inbox', compact('reports', 'categories', 'statuses'));
    }
    ...
    public function createStep2(Report $report)
    {
        ...
        $availableExpenses = \App\Models\Expense::where('amount', '>', 0)
            ->where(function ($q) use ($user, $report) {
                $q->where(function ($subQ) use ($user) {
                    $subQ->where('user_id', $user->id)
                         ->where('status', \App\Models\Expense::STATUS_READY);
                })->orWhereHas('report', function ($subQ) use ($user, $report) {
                    $subQ->where('user_id', $user->id)
                        ->where('id', '!=', $report->id)
                        ->whereIn('status', [
                            \App\Models\Report::STATUS_OBSERVED_BY_MANAGER,
                            \App\Models\Report::STATUS_OBSERVED_BY_REVIEWER,
                            \App\Models\Report::STATUS_RESUBMITTED_BY_REQUESTER_MANAGER,
                            \App\Models\Report::STATUS_RESUBMITTED_BY_REQUESTER_REVIEWER,
                        ]);
                });
            })
    ...
    public function toggleExpense(Request $request, Report $report, Expense $expense)
    {
        ...
        $expense->update([
            ...
            'status' => \App\Models\Expense::STATUS_ASSIGNED
        ]);
    ...
    public function addExpense(Report $report)
    {
        ...
        return view('reports.add-expense', compact('report', 'categories', 'availableExpenses'));
    }

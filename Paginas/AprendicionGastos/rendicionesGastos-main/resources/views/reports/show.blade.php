<x-layouts.app
    ...
    <div class="report-detail-minimal report-page app-main-card flex-1 flex flex-col h-full">
            ...
            <div class="report-layout pb-12">
                ...
                <div class="report-section">
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="report-expense-card bg-white p-6 space-y-6">
                            ...
                            <div class="flex flex-wrap items-center gap-3 pt-5 border-t border-slate-200">
                                ...
                                <a href="{{ asset('storage/' . $expense->attachment_path) }}" class="btn-sofofa" target="_blank" onclick="event.stopPropagation();">
                                ...
                                <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-auto">Doc: {{ $expense->document_type }}</span>
                                                            ...
                                                            <form method="POST" action="{{ route('expenses.destroy', $expense) }}" onsubmit="return confirm('¿Eliminar este gasto?')" onclick="event.stopPropagation()">
                                                                @csrf @method('DELETE')
                    ...
                    <div class="mt-8 flex justify-center">
                        <a href="{{ route('expenses.add', $report) }}" class="inline-flex items-center gap-3 px-8 py-4 bg-white border-2 border-sofofa-blue text-sofofa-blue font-black rounded-2xl hover:bg-blue-50 transition-all text-xs uppercase tracking-widest shadow-lg shadow-blue-500/10">
        ...
        <div class="report-side w-full space-y-6">
            ...
            <div class="report-panel bg-white rounded-3xl border-2 border-slate-100 p-6 shadow-sm space-y-3">
                ...
                <form method="POST" action="{{ route('reports.cancel', $report) }}" onsubmit="return confirm('¿Anular este informe? Esta acción no se puede deshacer.')">
                    @csrf
                ...
                <form method="POST" action="{{ route('reports.destroy', $report) }}" onsubmit="return confirm('¿Eliminar este informe permanentemente?')">
                    @csrf @method('DELETE')
            ...
            @elseif(!in_array($report->status, [\App\Models\Report::STATUS_REIMBURSED, \App\Models\Report::STATUS_CLOSED, \App\Models\Report::STATUS_CANCELLED]))
            <div class="report-panel bg-white rounded-3xl border-2 border-slate-100 p-6 shadow-sm">
                <form method="POST" action="{{ route('reports.cancel', $report) }}" onsubmit="return confirm('¿Anular este informe? Se notificará a los involucrados.')">
                    @csrf
                    <button type="submit" class="report-action-btn report-action-btn-neutral">
            ...
            <div class="report-panel bg-white rounded-3xl border-2 border-violet-100 p-6 shadow-sm">
                ...
                <form method="POST" action="{{ route('reports.enableEdit', $report) }}" onsubmit="return confirm('¿Habilitar edición para el solicitante?')">
                    @csrf
            ...
            <div class="report-panel bg-white rounded-3xl border-2 border-teal-100 p-8 shadow-xl shadow-teal-500/5 transition-all">
                ...
                <form action="{{ route('reports.pay', $report) }}" method="POST">
                    @csrf
        ...
        <div class="reject-modal-panel" role="dialog" aria-modal="true" aria-labelledby="reject-modal-title">
            ...
            <form action="{{ route('reports.reject', $report) }}" method="POST">
                ...
                <div class="reject-modal-footer">
                    ...
                    <button type="submit" formaction="{{ route('reports.approve', $report) }}" formmethod="POST" formnovalidate class="reject-modal-accept">Aceptar Solicitud</button>
    ...
    <div id="observe-modal" class="{{ $errors->has('comment') && old('_modal') === 'observe' ? '' : 'hidden' }} fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white rounded-[2rem] shadow-2xl max-w-md w-full relative border border-slate-100 overflow-hidden transform transition-all">
            ...
            <form action="{{ route('reports.observe', $report) }}" method="POST">
                @csrf

<x-layouts.app
                    ...
                    <tr class="hover:bg-slate-50/80 transition-all duration-200 group">
                        ...
                        <td class="px-8 py-6 text-right">
                            ...
                            <div class="inline-flex items-center gap-2">
                                ...
                                @if($canDelete)
                                    <form method="POST" action="{{ route('users.destroy', $user) }}" class="inline-flex" onsubmit="return confirm('¿Seguro que deseas eliminar al usuario {{ addslashes($user->name) }}?');">
                                        @csrf
                                        @method('DELETE')
    ...
    <div id="user-modal" class="user-modal-wrap hidden fixed flex">
        ...
        <div class="user-modal-panel relative z-10">
            ...
            <form id="user-form" action="{{ route('users.store') }}" method="POST" class="user-modal-form">
                @csrf
                ...
                <div class="user-field">
                    ...
                    <select name="role" id="user-role" required class="user-input appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236B7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22m6%208%204%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.75rem_center] bg-no-repeat pr-10">
                ...
                <div class="user-field">
                    ...
                    <select name="has_fixed_fund" id="user-has-fixed-fund" required onchange="toggleFixedFundAmount()" class="user-input appearance-none bg-[url('data:image/svg+xml;charset=utf-8,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20fill%3D%22none%22%20viewBox%3D%220%200%2020%2020%22%3E%3Cpath%20stroke%3D%22%236B7280%22%20stroke-linecap%3D%22round%22%20stroke-linejoin%3D%22round%22%20stroke-width%3D%221.5%22%20d%3D%22m6%208%204%204%204-4%22%2F%3E%3C%2Fsvg%3E')] bg-[length:1.25rem_1.25rem] bg-[right_0.75rem_center] bg-no-repeat pr-10">
    ...
    <script>
        ...
        function editUser(user) {
            ...
            document.getElementById('user-form').action = `/usuarios/${user.id}`;
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (hasErrors && oldMethod === 'PUT' && editingUserId) {
                ...
                document.getElementById('user-form').action = `/usuarios/${editingUserId}`;
                document.getElementById('form-method').value = 'PUT';
                document.getElementById('user-password').required = false;
                document.getElementById('password-section').classList.add('hidden');
                document.getElementById('submit-btn').innerText = 'Actualizar Datos';

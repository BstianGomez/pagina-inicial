...
class Report extends Model
{
    ...
    public function getHasDuplicateExpensesAttribute(): bool
    {
        foreach ($this->expenses as $expense) {
            ...
            $isDupe = \App\Models\Rendicion\Expense::where('amount', $expense->amount)
    ...
    public function getDuplicateReportsAttribute()
    {
        ...
        foreach ($this->expenses as $expense) {
            $dupes = \App\Models\Rendicion\Expense::where('amount', $expense->amount)
                ->where('provider_rut', $expense->provider_rut)
                ->where('id', '!=', $expense->id)
                ->pluck('report_id');
        ...
        return \App\Models\Rendicion\Report::whereIn('id', $duplicateReportIds->unique())->where('id', '!=', $this->id)->get();
    }

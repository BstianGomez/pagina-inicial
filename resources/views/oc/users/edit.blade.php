...
<body>
    <div class="page">
        <div class="card">
            ...
            <form method="POST" action="{{ route('oc.users.update', $user) }}">
                @csrf
                @method('PUT')
                <div class="field">
                ...
                <div class="field">
                    ...
                    @php
                        ...
                        $assignedApps = old('assigned_apps', $user->assigned_apps ?? ($user->assigned_app ? [$user->assigned_app] : []));
                        if (!is_array($assignedApps)) {
                    ...
                    @if($canEditRole)
                        <select class="select" id="role" name="role" required>
                            @if($currentUser->isSuperAdmin())
                                @php $currentRole = $user->role ?? $user->rol; @endphp
                                <option value="super_admin" {{ old('role', $currentRole) === 'super_admin' ? 'selected' : '' }}>Super Admin</option>
                                <option value="admin" {{ old('role', $currentRole) === 'admin' ? 'selected' : '' }}>Admin</option>
                                <option value="gestor" {{ old('role', $currentRole) === 'gestor' ? 'selected' : '' }}>Gestor</option>
                                <option value="cliente" {{ old('role', $currentRole) === 'cliente' ? 'selected' : '' }}>Cliente</option>
                    ...
                    @else
                        <div style="padding: 10px; background: #f3f4f6; border-radius: 8px; font-size: 14px; color: var(--muted);">
                            Rol: <strong>{{ ucfirst($user->role ?? $user->rol) }}</strong>

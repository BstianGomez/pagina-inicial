...
<body>
<main class="card"><div class="top"></div><section class="content"><div class="code">404</div><h1>Página no encontrada</h1><p>La URL solicitada no existe o fue movida.</p><div class="actions"><a href="{{ request()->is('rendicion*') ? route('rendicion.dashboard') : (request()->is('viajes*') ? url('/viajes') : (request()->is('oc*') ? route('oc.index') : route('app-redirect'))) }}" class="btn btn-primary">Ir al inicio</a><a href="{{ url()->previous() }}" class="btn btn-secondary">Volver</a></div></section></main>
</body>
</html>

<x-rendicion.layouts.app
    ...
    <style>
        ...
        #step2-dot  { background: #0f6bb6; color: #fff; cursor: default; }
    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center space-x-4">
                    <a href="{{ route('rendicion.expenses.createStep1', $report->id) }}" title="Volver a información general"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <a href="{{ route('rendicion.dashboard') }}" class="text-sm font-bold text-slate-400 hover:text-red-500 transition-colors">Cancelar</a>
            <span id="no-next-hint" class="text-xs text-slate-400 font-semibold">Selecciona al menos un gasto para continuar.</span>

            <form action="{{ route('rendicion.expenses.storeStep2', $report) }}" method="POST" id="form-submit-report" class="m-0">
                @csrf
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div class="flex items-center gap-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full lg:w-auto">
                    <select id="available-sort" class="w-full rounded-lg border-slate-200 text-sm focus:border-sofofa-blue focus:ring-sofofa-blue">
                        <option value="newest" selected>Mas nuevos primero</option>
                        <option value="oldest">Mas antiguos primero</option>
                    </select>
                    <input id="available-query" type="text" placeholder="Filtrar por motivo, proveedor, categoria o CECO" class="w-full rounded-lg border-slate-200 text-sm focus:border-sofofa-blue focus:ring-sofofa-blue">
                </div>
            </div>
    ...
    @php
        ...
        $allExpenses = $assignedExpenses->map(fn($e) => ['id'=>$e->id,'assigned'=>true,'amount'=>$e->amount,'url'=>route('rendicion.reports.toggleExpense',[$report,$e]),'cat'=>$e->category->name??'Gasto','ceco'=>$e->ceco->code??'—','reason'=>$e->reason,'provider'=>$e->provider_name,'date'=>\Carbon\Carbon::parse($e->expense_date)->format('d/m/Y'),'ts'=>optional($e->expense_date)->timestamp ?? 0,'doctype'=>$e->document_type,'from'=>''])
            ->concat($availableExpenses->map(fn($e) => ['id'=>$e->id,'assigned'=>false,'amount'=>$e->amount,'url'=>route('rendicion.reports.toggleExpense',[$report,$e]),'cat'=>$e->category->name??'Gasto','ceco'=>$e->ceco->code??'—','reason'=>$e->reason,'provider'=>$e->provider_name,'date'=>\Carbon\Carbon::parse($e->expense_date)->format('d/m/Y'),'ts'=>optional($e->expense_date)->timestamp ?? 0,'doctype'=>$e->document_type,'from'=>$e->report->title??'—']));
    @endphp
    ...
    (function () {
        ...
        const availableSort  = document.getElementById('available-sort');
        const availableQuery = document.getElementById('available-query');
        ...
        function sortExpenses(expenses) {
            const mode = availableSort ? availableSort.value : 'newest';
            return [...expenses].sort((a, b) => {
                const diff = Number(a.ts || 0) - Number(b.ts || 0);
                if (mode === 'oldest') {
        ...
        function renderAll() {
            ...
            const query = normalizeText(availableQuery ? availableQuery.value : '');

            const assignedExpenses = sortExpenses(Object.values(state).filter(exp => exp.assigned));
            const allAvailableExpenses = sortExpenses(Object.values(state).filter(exp => !exp.assigned));
            const visibleAvailableExpenses = allAvailableExpenses.filter(exp => {
                if (!query) {
                    return true;
                }
                const haystack = normalizeText([exp.reason, exp.provider, exp.cat, exp.ceco, exp.from, exp.doctype].join(' '));
            ...
            visibleAvailableExpenses.forEach(exp => {
            ...
            if (availableNum === 0 && assignedNum === 0) {
                ...
                availableList.innerHTML = '<div class="py-14 text-center"><p class="text-sm font-bold text-slate-400">No hay resultados con el filtro aplicado.</p></div>';
            }

            assignedCount.textContent  = assignedNum;
            availableCount.textContent = query ? `${availableNum}/${allAvailableExpenses.length}` : availableNum;
        ...
        if (availableSort) {
            availableSort.addEventListener('change', renderAll);
        }
        if (availableQuery) {
            availableQuery.addEventListener('input', renderAll);
        }
